...
1. **在当前训练设置下，直接堆叠原始 NIR/RE 波段的效果不稳定：** 60 epoch、partial_random 初始化条件下，MSI（76.75%）低于 RGB baseline（80.02%），表明原始多光谱波段的利用效果可能受训练时长、初始化策略等因素影响，尚未充分发挥其潜力。

2. **植被指数（尤其是 NDVI）对多光谱输入的有效利用具有重要作用：** RGB+VI 在相同5通道数下优于 MSI（81.10% vs 76.75%），表明 NDVI/NDRE 提供了更直接的植被判别信息；通道敏感性分析进一步显示，NDVI 对整体分割的影响最大，而 NDRE 在当前7通道组合中可能与其他通道存在一定程度的信息重叠。

3. **原始光谱波段与植被指数在当前实验设置下表现出互补性：** MSI+VI（82.38%）优于 RGB+VI（81.10%），表明二者结合的7通道输入在本实验设置下取得了更好的结果，但这一互补性是否普遍适用仍有待进一步验证。

4. **CE-Dice 损失函数在 MSI+VI 输入上带来了明显的性能提升：** 相同输入和训练设置下，从 CE（82.38%）到 CE-Dice（84.20%），mIoU 提升 1.82%、crop IoU 提升 3.74%，提示区域重叠优化目标对杂草分割任务的类别不平衡问题有一定缓解作用。

5. **通道敏感性分析显示 RE 和 NDVI 对模型预测影响较大：** 基于均值替换扰动实验，RE 替换后 weed IoU 下降幅度最大，NDVI 替换后整体 mIoU 下降幅度最大；该结果与多光谱输入的物理意义一致，但需注意该分析基于单个 checkpoint，且通道间存在信息耦合，结果不等同于因果贡献。
...
**分析方法：** 固定训练好的 M4 模型（`best.pth`），逐一将各输入通道替换为该通道在测试集上的均值，比较扰动前后的 mIoU 和 weed IoU，用于分析模型对不同输入通道的敏感程度。

| 通道 | ΔmIoU（替换后下降） | Δweed IoU | 说明 |
|---|---|---|---|
| **NDVI** | **-30.58%** | -25.21% | 整体 mIoU 最敏感通道 |
| **G** | -26.49% | -33.31% | 整体和 weed 均高度敏感 |
| **RE** | -23.21% | **-35.16%** | weed IoU 最敏感通道 |
| R | -9.34% | -10.72% | 中等敏感 |
| NIR | -2.22% | -2.93% | 独立影响有限 |
| B | -0.86% | -0.51% | 敏感性最低 |
| NDRE | +1.11% | +1.38% | 替换后性能略有提升，提示信息冗余 |

**主要发现：**

- **NDVI** 替换造成整体 mIoU 最大下降，表明模型整体分割对 NDVI 较为敏感；
- **RE（Red Edge）** 替换造成 weed IoU 最大下降，表明红边波段对模型中的杂草识别较为重要；
- **G 波段**同样表现出较高敏感性（整体和 weed 均排前三），不能简单认为 RGB 通道贡献较低；
- **NIR** 独立影响有限，可能部分信息已由 NDVI 所表达；
- **NDRE** 替换后指标略有提高，提示其在当前 MSI+VI 输入组合中可能存在信息冗余。

**结论：** 在当前 M4 模型和测试集扰动实验下，NDVI 对整体分割最敏感，RE 对杂草识别最敏感，G 波段同样具有较高影响；NIR 独立增益有限，NDRE 可能存在冗余。
